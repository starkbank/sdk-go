...
func Query(params map[string]interface{}, user user.User) chan BoletoPayment {
	//	Retrieve BoletoPayment structs
	//
	//	Receive a generator of BoletoPayment structs previously created in the Stark Bank API
	//
	//	Parameters (required):
	//	- user [Organization/Project struct, default nil]: Organization or Project struct. Not necessary if starkbank.user was set before function call
	//
	...
	//
	//	Return:
	//	- Generator of BoletoPayment structs with updated attributes
...
func Page(params map[string]interface{}, user user.User) ([]BoletoPayment, string, Error.StarkErrors) {
	...
	//
	//	Receive a list of up to 100 BoletoPayment structs previously created in the Stark Bank API and the cursor to the next page.
