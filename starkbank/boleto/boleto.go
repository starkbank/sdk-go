...
func Query(params map[string]interface{}, user user.User) (chan Boleto, chan Error.StarkErrors) {
	...
	boletosError := make(chan Error.StarkErrors)
	...
	go func() {
		for content := range query {
			...
			if err != nil {
				boletosError <- Error.UnknownError(err.Error())
				continue
			}
			boletos <- boleto
		}
